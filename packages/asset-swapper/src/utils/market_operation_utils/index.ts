...
export class MarketOperationUtils {
    ...
    public async getMarketSellOrdersAsync(
        ...
        const [orderFillableAmounts, ethToMakerAssetRate, dexQuotes] = await this._sampler.executeAsync(
    ...
    public async getMarketBuyOrdersAsync(
        ...
        const [makerToken, takerToken] = getNativeOrderTokens(nativeOrders[0]);
        // Call the sampler contract.
        const [orderFillableAmounts, ethToTakerAssetRate, dexQuotes] = await this._sampler.executeAsync(
            // Get native order fillable amounts.
    ...
    public async getBatchMarketBuyOrdersAsync(
        ...
        const ops = [
            ...
            ...batchNativeOrders.map((orders, i) =>
                DexOrderSampler.ops.getBuyQuotes(sources, getNativeOrderTokens(orders[0])[0], getNativeOrderTokens(orders[0])[1], [
                    makerAmounts[i],
                ]),
            ),
        ];
    ...
    private _generateOptimizedOrders(opts: {
        ...
        const { makerToken, takerToken, side, inputAmount } = opts;
        ...
        const paths = createFillPaths({
            ...
            orders: createSignedOrdersWithFillableAmounts(
                side,
                opts.nativeOrders,
        ...
        if (opts.allowFallback) {
            fallbackPath = findOptimalPath(
                side,
                getUnusedSourcePaths(optimalPath, paths),
                inputAmount,
            ) || [];
        }
        return createOrdersFromPath(
