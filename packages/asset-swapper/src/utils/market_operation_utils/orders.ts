...
export function createOrdersFromPath(
    path: Fill[],
...
function getBridgeAddressFromSource(
...
function createCommonBridgeOrderFields(
    ...
    return {
        ...
        expirationTimeSeconds: new BigNumber(
            Math.floor(Date.now() / ONE_SECOND_MS) + ONE_HOUR_IN_SECONDS,
        ),
        makerFeeAssetData: NULL_BYTES,
        takerFeeAssetData: NULL_BYTES,
        makerFee: ZERO_AMOUNT,
